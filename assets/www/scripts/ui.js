(function(window, document, Kinvey, $, Handlebars) {
  ...
  document_.on({
    ...
    deviceready: function() {
      // Set switch to correct value on initialization.
  ...
  /**
   * Settings screen. Contains information about this app, allows push
   * configuration.
   */
  var settings = $('#settings');
  settings.on('pageinit', function() {
    // Add switch controls.
    var flag = settings.find('[name="switch"]');
    flag.on('change', function(e) {
      ...
      // Save.
      var action = "1" === flag.val() ? 'registerDevice' : 'unregisterDevice';
      window.plugins.pushNotification[action](function() {
        flag.slider('enable');
        flag.loading(false);
      });
    });
